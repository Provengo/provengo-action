...
⚠️ NOTE: In the below listings, we assume that `provengo` is in the system's PATH variable, and that `/Users/hadas/IdeaProjects/SeleniumBasedTests/examples/a-a` is the path to this directory.
...
Perform a single run through the specification. Good for "Sanity checks", i.e. to see examples of what can happen.
    ...
    provengo run --dry /Users/hadas/IdeaProjects/SeleniumBasedTests/examples/a-a
...
Draw the specification in a PDF file.
    ...
    provengo analyze -f pdf /Users/hadas/IdeaProjects/SeleniumBasedTests/examples/a-a
...
Sample 10 scenarios into a file. The scenarios are stored in a file called `samples.json` (this can be changed using the `-o`/`--output-file` switch).
    ...
    provengo sample --overwrite --size 10 /Users/hadas/IdeaProjects/SeleniumBasedTests/examples/a-a
...
**Requires running `sample` first** (the previous command)**.**
    ...
    provengo ensemble --size 5 /Users/hadas/IdeaProjects/SeleniumBasedTests/examples/a-a
...
Draw the specification, and highlight the traces in the optimized test suite create by the previous command.
    ...
    provengo analyze -f pdf --highlight ensemble.json /Users/hadas/IdeaProjects/SeleniumBasedTests/examples/a-a

### Create Test Scripts for Third Party Systems

Converts the runs in `ensemble.json` to automation test scripts.

    provengo gen-scripts -s ensemble.json /Users/hadas/IdeaProjects/SeleniumBasedTests/examples/a-a
